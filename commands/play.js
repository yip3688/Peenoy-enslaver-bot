...
module.exports = {
    name: "play",
    description: "Plays songs, retard",
    usage: "[song]",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **Silli, silli nigga, you need be in a vc to play something**");
        if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, ":x: | **UwU not same vc silli**");
       ...
       return client.sendTime(message.channel,"❌ | **Lavalink node not connected**");
        ...
        if (!player) return client.sendTime(message.channel, "❌ | **Sussy baka nothing palying**");
